...
CREATE CONSTRAINT city_name_state IF NOT EXISTS FOR (c:City) REQUIRE (c.name, c.state) IS UNIQUE;
...
CALL {
  ...
  // Chiave composta (name + state) per gestire città omonime (diverse) 
...
CALL {
  ...
  LOAD CSV WITH HEADERS FROM url AS row
  MATCH (city:City {name: row.city, state: row.state}) 
...
MATCH p=()-[r:DELIVERED_TO]->() RETURN p LIMIT 25
